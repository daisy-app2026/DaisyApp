...
const TalkToPastHome: React.FC = () => {
  ...
  const handleCardPress = (session: TalkToPastSession) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await deleteTalkToPastSession(deleteConfirmId);
    ...
    <View style={styles.container}>
      ...
      {sessions.length === 0 ? (
        <View style={styles.emptyState}>
          <Ionicons name="time-outline" size={52} color="rgba(45,90,27,0.25)" />
          <Text style={styles.emptyTitle}>No reflections yet</Text>
          <Text style={styles.emptySubtitle}>
            Tap + to begin your first guided journey
          </Text>
        </View>
      ) : (
        <ScrollView
        ...
        >
          <View style={styles.pastReflectionsSection}>
            {sessions.map((session) => {
              ...
              const personIcon = getPersonIcon(session.personName);
          ...
          <Text style={styles.bottomLabel}>Each new person starts fresh</Text>
        </ScrollView>
      )}
